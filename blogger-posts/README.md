...
5. ⚠️ **게시 상태를 '프로덕션'으로 바꾸세요** (같은 화면의 *앱 게시* 버튼)
   - **'테스트' 상태로 두면 리프레시 토큰이 7일 뒤 자동 만료됩니다.**
     일주일쯤 지나 갑자기 발행이 안 되면 대부분 이것 때문입니다.
   - 본인만 쓰는 앱이라 심사 없이 바로 전환됩니다.
     ("확인되지 않은 앱" 경고가 떠도 *고급 → 이동* 으로 진행하면 됩니다.)
   - 이미 '테스트'로 토큰을 받았다면, 프로덕션으로 바꾼 뒤 ④를 다시 하세요.
...
Actions 로그의 마지막 줄을 먼저 보세요. 대부분 아래 셋 중 하나입니다.

| 로그에 나오는 말 | 원인 | 할 일 |
|---|---|---|
| `skipping: Blogger credentials not configured yet` | 비밀값 3개가 아직 없음 | ⑤ 를 마치세요 |
| `token refresh failed (400) ... invalid_grant` | 토큰 만료(대개 '테스트' 상태 7일 제한) | ②-5 로 프로덕션 전환 후 ④ 재발급 |
| `blog not found` | 블로그 주소가 다름 | Variables 에 `BLOGGER_BLOG_URL` 등록 |

설정을 마친 뒤 바로 확인하려면 Actions → **Publish to Blogger** → **Run workflow**
로 수동 실행해 보세요. 글을 다시 올릴 필요는 없습니다.
